...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for j in range(len(sizes) - 1):
        if j < len(sizes) - 2:
            ...
            activ_func = output_activation
        mlp_layers += [nn.Linear(sizes[j], sizes[j + 1]), activ_func()]
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        rew = np.append(self.rew_buf[path_slice], last_val)
        val = np.append(self.val_buf[path_slice], last_val)
        ...
        self.ret_buf[path_slice] = discount_cumsum(rew, self.gamma)[:-1]
...
class Agent:
    def __init__(self, env, activation=nn.Tanh):
        ...
        self.l = 3  # layer number of networks, orig = 2
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi, logp = self.actor.forward(state)
    ...
    def get_action(self, obs):
        ...
        act = self.actor._distribution(torch.as_tensor(obs, dtype=torch.float32)).sample()
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        dist = agent.actor._distribution(data['obs'])
        logp = agent.actor._log_prob_from_distribution(dist, data['act'])
        policy_loss = (-data['tdres'] * logp).mean()
