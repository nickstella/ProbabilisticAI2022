...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel = Matern(nu=0.5, length_scale=1.0)
        self.model = GaussianProcessRegressor(kernel=self.kernel, optimizer='fmin_l_bfgs_b', random_state=None,
        ...
        #self.kernel = 0.2 * WhiteKernel(noise_level=0.5) + 0.8 * Matern(nu=0.5, length_scale=1.0)
        #self.model = GaussianProcessRegressor(kernel=self.kernel, optimizer=None)
        #self.model = GaussianProcessRegressor(kernel=self.kernel, n_restarts_optimizer=0, normalize_y=False,
    ...
    def fitting_model(self, train_GT: np.ndarray,train_features: np.ndarray):
        ...
        print('Fitting with Matern kernel')
        #print('Fitting with RBF kernel')
        #print('Fitting with WhiteNoise kernel')
