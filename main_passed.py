...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi, logp_a = self.actor.forward(state)
    ...
    def get_action(self, obs):
        ...
        action = self.actor._distribution(torch.as_tensor(obs, dtype=torch.float32)).sample()

        '''obs_tensor = torch.from_numpy(obs)
        dtype = next(iter(self.actor.parameters())).dtype
        return self.actor._distribution(obs_tensor.to(dtype))[0]#.numpy()'''
        return action
