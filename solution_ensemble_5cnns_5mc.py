...
class SelfMadeNetwork(nn.Module):
    def __init__(self,
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(output_size=(3, 3))

        self.fc = nn.Sequential(
            nn.Linear(32 * 3 * 3, 512),  # (in_features=32 * 3 * 3, out_features=512),
            nn.ReLU(inplace=True),
            nn.Dropout(p=dropout_p),

            nn.Linear(512, 10),  # (in_features=512, out_features=10),
            nn.LogSoftmax(dim=1)
        )
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train, dropout_p,
                 *args, **kwargs):
        super().__init__(dataset_train, *args, **kwargs)
        ...
        # self.network = MNISTNet(in_features=28*28,out_features=10)
        self.network = SelfMadeNetwork(dropout_p=self.dropout_p)
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True
        )
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        for i in range(num_sample):
            self.network.eval()
            mc_e = F.softmax(self.network(x), dim=1)
            montecarlo_list.append(mc_e)
        mc_torch = torch.stack(montecarlo_list)
        estimated_probability = torch.mean(mc_torch, 0)
...
class EnsembleTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.EnsembleNetworks.append(
            {'network_name': 'CNN1', 'network': SelfMadeNetwork(dropout_p=0.45), 'num_epochs': 20,
             'learning_rate': 1e-3, 'batch_size': 128})
        self.EnsembleNetworks.append(
            {'network_name': 'CNN2', 'network': SelfMadeNetwork(dropout_p=0.4), 'num_epochs': 20,
             'learning_rate': 1e-3, 'batch_size': 64})
        self.EnsembleNetworks.append(
            {'network_name': 'CNN3', 'network': SelfMadeNetwork(dropout_p=0.4), 'num_epochs': 20,
             'learning_rate': 1e-3, 'batch_size': 128})
        self.EnsembleNetworks.append(
            {'network_name': 'CNN4', 'network': SelfMadeNetwork(dropout_p=0.5), 'num_epochs': 25,
             'learning_rate': 1e-3, 'batch_size': 64})
        self.EnsembleNetworks.append(
            {'network_name': 'CNN5', 'network': SelfMadeNetwork(dropout_p=0.5), 'num_epochs': 25,
             'learning_rate': 1e-3, 'batch_size': 128})
        self.EnsembleNetworks.append(
            {'network_name': 'MC1', 'network': SelfMadeNetwork(dropout_p=0.45),
                'num_epochs': 20, 'learning_rate': 1e-3, 'batch_size': 128})
        self.EnsembleNetworks.append(
            {'network_name': 'MC2', 'network': SelfMadeNetwork(dropout_p=0.4),
                'num_epochs': 20, 'learning_rate': 1e-3, 'batch_size': 64})
        self.EnsembleNetworks.append(
            {'network_name': 'MC3', 'network': SelfMadeNetwork(dropout_p=0.4),
                'num_epochs': 20, 'learning_rate': 1e-3, 'batch_size': 128})
        self.EnsembleNetworks.append(
            {'network_name': 'MC4', 'network': SelfMadeNetwork(dropout_p=0.5),
                'num_epochs': 25, 'learning_rate': 1e-3, 'batch_size': 64})
        self.EnsembleNetworks.append(
            {'network_name': 'MC5', 'network': SelfMadeNetwork(dropout_p=0.5),
                'num_epochs': 25, 'learning_rate': 1e-3, 'batch_size': 128})

        # self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.learning_rate)

    def train(self):

        for model in self.EnsembleNetworks:
            ...
            print(f'### Training model {network_name} ###')
            ...
            for _ in progress_bar:
                for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                    # batch_x are of shape (batch_size, 1, 28, 28 (28*28=784)), batch_y are of shape (batch_size,)
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for model in self.EnsembleNetworks:
            ...
            if network_name.startswith('CNN'):
                network.eval()
                preds = F.softmax(network(x), dim=1)
                model_prob.append(preds)

            if network_name.startswith('MC'):
                ...
                for i in range(num_sample):
                    network.eval()
                    mc_e = F.softmax(network(x), dim=1)
                    montecarlo_list.append(mc_e)
                mc_torch = torch.stack(montecarlo_list)
                preds = torch.mean(mc_torch, 0)
                model_prob.append(preds)
        ...
        weight_m = 1 / len(model_prob)
        estimated_probability = 0

        for pred in model_prob:
            estimated_probability += pred * weight_m
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        #  )
        self.weights_var_posterior = nn.Parameter(torch.Tensor(self.input_dim, self.output_dim).uniform_(-0.01, 0.01))
...
class SelfTrainer(Framework):
    def __init__(self, dataset_train, print_interval=5, *args, **kwargs):
        """
        ...
        self.network = SelfMadeNetwork(dropout_p=0.4)
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True
        )
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.learning_rate)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                # batch_x are of shape (batch_size, 1, 28, 28), batch_y are of shape (batch_size,)
