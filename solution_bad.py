...
class BO_algo():
    def __init__(self):
        ...
        kernel3 = WhiteKernel(noise_level=0.5)  # 0.4, 0.5
        self.kernel_f = Sum(Product(kernel1, kernel2), kernel3)
        #self.kernel_f = Product(kernel1, kernel2)
        self.f_map = GaussianProcessRegressor(kernel=self.kernel_f)
        ...
        #self.kernel_v = Product(vkernel1, vkernel2)
        # Default GaussianProcessRegressor (check argument possibilities)
        self.v_map = GaussianProcessRegressor(kernel=self.kernel_v)
    ...
    def acquisition_function(self, x):
        ...
        PI_v = norm.cdf(SAFETY_THRESHOLD + trade_off, loc=mu_v, scale=sigma_v)
        ...
        if PI_v < .5:
            ...
            a = (mu_f - opt_f - trade_off) / sigma_f
    ...
    def get_solution(self):
        ...
        trade_off = 0.01
        filter = (self.past_points[:, 2] > SAFETY_THRESHOLD + trade_off)
